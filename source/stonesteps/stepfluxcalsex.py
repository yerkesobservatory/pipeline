...
class StepFluxCalSex(StepParent):
    ...
    def setup(self):
        ...
        self.paramlist.append(['delete_cal',False,
    ...
    def run(self):
        ...
        if not os.path.exists(self.datain.filename) :
            self.datain.save()
        # Make command string
        command = self.getarg('sxcmd') % (self.datain.filename)
        command += ' ' + self.getarg('sxoptions')
        # Call process
        ...
        seo_catalog = astropy.table.Table.read('test.cat', format="fits", hdu='LDAC_OBJECTS')

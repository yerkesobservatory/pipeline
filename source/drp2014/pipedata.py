...
#import pyfits # pyfits library @UnresolvedImport
...
try: # import fits library
    from astropy.io import fits # astropy.io.fits
except:
    import pyfits as fits # pyfits library @UnresolvedImport
...
class PipeData(object):
    ...
    def __getattr__(self, name):
        ...
        if name == 'filenamebase':
            ...
            if extloc < 0 or not (fname[extloc:] in '.fits.fts.fit'):
                extloc = len(fname)
                self.log.warn('Filename has non-fits extension')
            typeloc = fname[0:extloc].rfind('.')
            if typeloc < 0: typeloc = extloc
            if len(fpath) == 0:
                return fname[:typeloc]
            else:
    ...
    def save(self,filename = None):
        ...
        if filename == None:
            filename = self.filename
        # create valid obs_id
        obsid='P_HAWCPIPEv'+PipeData.pipever
        obsid+='_'+time.strftime('%Y-%m-%d_%H:%M:%S')
        self.setheadval('OBS_ID',obsid,'Observation identification')
        # update pipeline keywords
        self.setheadval('PIPEVERS','HAWC Pipe v'+PipeData.pipever,
                        'Pipeline Version')
        ### save file
        ...
        if 'COMPLETE' in hdul[0].header.keys(): # Remove complete flag
            del hdul[0].header['COMPLETE']

        # make hdus for tables
        for i in range(len(self.tabnames)):
            hdut = fits.new_table(self.tabdata[i],self.tabheads[i])
    ...
    def mergehead(self, other):
        ...
        """
        # get self and other cards and parameters
        othercards=other.header.ascardlist()
        otherpars=[othercards[name].value for name in othercards.keys()
                   if name.find('PARENT') == 0] # parent values for other
        selfcards=self.header.ascardlist()
        selfpars=[selfcards[name].value for name in selfcards.keys()
                  if name.find('PARENT') == 0] # parent values for self
        # merge parent keywords from other file (no duplicates)
        parinds=[string.atoi('0'+name[6:]) for name in selfcards.keys()
                 if name.find('PARENT') == 0]
        if len(parinds) > 0 :
            parind = numpy.array(parinds).max() # top parent index
        else:
            parind = 0
        for parent in otherpars: # loop through other parents
            if not parent in selfpars: # only add if not duplicate
                parind+=1
                self.header['PARENT%d' % parind] = (parent, '')
        # add history keywords (no duplicates)
        otherhist=[othercards[ind].value for ind in range(len(othercards))
                   if othercards[ind].key=='HISTORY'] # hist values for other
        selfhist=[selfcards[ind].value for ind in range(len(selfcards))
                  if selfcards[ind].key=='HISTORY'] # hist values for self
    ...
    def copydata(self, other, dataname):
        ...
        isimg = 0
        if dataname in other.imgnames:
            isimg = 1
            othind = other.imageindex(dataname)
        elif dataname in other.tabnames:
        ...
        if dataname in self.imgnames:
            if isimg < 1:
                self.imagedel(dataname)
        elif dataname in self.tabnames:
            if isimg > 0:
    ...
    def tableaddcol(self, colname, array, tablename = None, dtype=None):
        ...
        ### Else (table exists) base new table on old table
            ...
            if len(table) != len(array):
                self.log.error('TableAddCol: column array length != table length')
                raise ValueError('column array has length != table length')
    ...
    def tabledelcol(self, colname, tablename = None):
        ...
        # else -> remove column(s) from table
            ...
            if isinstance(table,fits.FITS_rec):
            ...
            else: # assume table is a regular record array
    ...
    def tablemergerows(self, rows):
        ...
        for colname in rows.dtype.names:
            # get merge function
            try:
                funct = self.config['table'][colname].lower()
            except :
    ...
    def tablemergetables(self,tables):
        ...
        for n,f,d,u in zip(names,formats,dims,units):
            # get merge function
            try:
                funct = self.config['table'][n].lower()
            except :
                # if keyword is not available
    ...
    def getheadval(self, key, dataname = ''):
        ...
        try:
            ...
            if val[0].isupper() and val[:2] not in ['T ', 'F ']:
                ...
                # make it a pyfits.Card then get value
                card = fits.Card()
                card.fromstring(key.upper + '=' + val)
                # Following code is unused because non-string values
                #   don't get converted correctly. However the new code
                #   may not work with all version of pyfits - tbd
                #tmp = val.split('/')
                #if len(tmp) > 1:
        ...
        if isinstance(val, fits.core.Undefined):
    ...
    def setheadval(self, key, value, comment=None, dataname = ''):
        ...
        # Set primay header
        if dataname == '':
            pass #self.header[key] = (value,comment)
        # Set image header
        elif dataname.upper() in self.imgnames:
            index = self.imageindex(dataname)
            self.imgheads[index][key] = (value, comment)
        # Set table header
        elif dataname.upper() in self.tabnames:
            index = self.tableindex(dataname)
            self.tabheads[index][key] = (value, comment)
        # Dataname not found -> return error message
        else:
            msg = 'Invalid data name (%s)---' % dataname
    ...
    def test(self):
        ...
        self.log.debug("Complete=%d" % self.getheadval('COMPLETE'))
